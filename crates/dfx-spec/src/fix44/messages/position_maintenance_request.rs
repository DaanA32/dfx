...
#[derive(Clone, Debug)]
pub struct PositionMaintenanceRequest<'a> {
    inner: Cow<'a, Message>
}

impl<'a> PositionMaintenanceRequest<'a> {
    //TODO implement
    ...
    pub fn adjustment_type<'b: 'a>(&'b self) -> Option<AdjustmentType<'b>> {
        self.inner.get_field(AdjustmentType::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_adjustment_type<'b: 'a>(&mut self, adjustment_type: AdjustmentType<'b>) {
        self.inner.to_mut().set_field(adjustment_type);
    }
        
    pub fn contrary_instruction_indicator<'b: 'a>(&'b self) -> Option<ContraryInstructionIndicator<'b>> {
        self.inner.get_field(ContraryInstructionIndicator::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_contrary_instruction_indicator<'b: 'a>(&mut self, contrary_instruction_indicator: ContraryInstructionIndicator<'b>) {
        self.inner.to_mut().set_field(contrary_instruction_indicator);
    }
