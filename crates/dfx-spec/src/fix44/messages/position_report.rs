...
/// PositionReport
#[derive(Clone, Debug)]
pub struct PositionReport<'a> {
    inner: Cow<'a, Message>
}

impl<'a> PositionReport<'a> {
    //TODO implement
    ...
    pub fn delivery_date<'b: 'a>(&'b self) -> Option<DeliveryDate<'b>> {
        self.inner.get_field(DeliveryDate::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_delivery_date<'b: 'a>(&mut self, delivery_date: DeliveryDate<'b>) {
        self.inner.to_mut().set_field(delivery_date);
    }
