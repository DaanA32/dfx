...
#[derive(Clone, Debug)]
pub struct CollateralAssignment<'a> {
    inner: Cow<'a, Message>
}

impl<'a> CollateralAssignment<'a> {
    //TODO implement
    ...
    pub fn coll_asgn_ref_id<'b: 'a>(&'b self) -> Option<CollAsgnRefId<'b>> {
        self.inner.get_field(CollAsgnRefId::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_coll_asgn_ref_id<'b: 'a>(&mut self, coll_asgn_ref_id: CollAsgnRefId<'b>) {
        self.inner.to_mut().set_field(coll_asgn_ref_id);
    }
