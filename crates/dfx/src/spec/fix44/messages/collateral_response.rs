...
/// CollateralResponse
#[derive(Clone, Debug)]
pub struct CollateralResponse<'a> {
    inner: Cow<'a, Message>
}

impl<'a> CollateralResponse<'a> {
    //TODO implement
    ...
    pub fn coll_req_id<'b: 'a>(&'b self) -> Option<CollReqId<'b>> {
        self.inner.get_field(CollReqId::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_coll_req_id<'b: 'a>(&mut self, coll_req_id: CollReqId<'b>) {
        self.inner.to_mut().set_field(coll_req_id);
    }
        
    pub fn coll_asgn_reason<'b: 'a>(&'b self) -> Option<CollAsgnReason<'b>> {
        self.inner.get_field(CollAsgnReason::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_coll_asgn_reason<'b: 'a>(&mut self, coll_asgn_reason: CollAsgnReason<'b>) {
        self.inner.to_mut().set_field(coll_asgn_reason);
    }
    ...
    pub fn coll_asgn_id<'b: 'a>(&'b self) -> Option<CollAsgnId<'b>> {
        self.inner.get_field(CollAsgnId::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_coll_asgn_id<'b: 'a>(&mut self, coll_asgn_id: CollAsgnId<'b>) {
        self.inner.to_mut().set_field(coll_asgn_id);
    }
        
    pub fn coll_asgn_trans_type<'b: 'a>(&'b self) -> Option<CollAsgnTransType<'b>> {
        self.inner.get_field(CollAsgnTransType::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_coll_asgn_trans_type<'b: 'a>(&mut self, coll_asgn_trans_type: CollAsgnTransType<'b>) {
        self.inner.to_mut().set_field(coll_asgn_trans_type);
    }
        
    pub fn coll_resp_id<'b: 'a>(&'b self) -> Option<CollRespId<'b>> {
        self.inner.get_field(CollRespId::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_coll_resp_id<'b: 'a>(&mut self, coll_resp_id: CollRespId<'b>) {
        self.inner.to_mut().set_field(coll_resp_id);
    }
        
    pub fn coll_asgn_resp_type<'b: 'a>(&'b self) -> Option<CollAsgnRespType<'b>> {
        self.inner.get_field(CollAsgnRespType::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_coll_asgn_resp_type<'b: 'a>(&mut self, coll_asgn_resp_type: CollAsgnRespType<'b>) {
        self.inner.to_mut().set_field(coll_asgn_resp_type);
    }
        
    pub fn coll_asgn_reject_reason<'b: 'a>(&'b self) -> Option<CollAsgnRejectReason<'b>> {
        self.inner.get_field(CollAsgnRejectReason::tag()).map(|v| v.try_into().ok()).flatten()
    }
    pub fn set_coll_asgn_reject_reason<'b: 'a>(&mut self, coll_asgn_reject_reason: CollAsgnRejectReason<'b>) {
        self.inner.to_mut().set_field(coll_asgn_reject_reason);
    }
